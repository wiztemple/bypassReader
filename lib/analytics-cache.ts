...
const initialAnalytics: BypassAnalytics = {
  ...
  serviceStats: {
    ...
    archivebuttons: { attempts: 0, successes: 0, failures: 0 },
  },
  domainStats: {},
};
...
export function useBypassAnalytics() {
    ...
    setAnalytics((prev) => {
      ...
      if (!domainStats[domain]) {
        ...
        const totalSuccesses = Math.min(
          (domainStats[domain].successRate / 100) *
            domainStats[domain].attempts +
            1,
          domainStats[domain].attempts
        );
        const newSuccessRate =
          (totalSuccesses / domainStats[domain].attempts) * 100;
        ...
        const serviceSuccessRate = Math.min(
          (serviceStats[service].successes / serviceStats[service].attempts) *
            100,
          100
        );
        const currentBestRate =
          (serviceStats[domainStats[domain].bestService]?.successes /
            serviceStats[domainStats[domain].bestService]?.attempts) *
            100 || 0;
  ...
  const recordFailure = (domain: string, service: string) => {
    setAnalytics((prev) => {
      ...
      if (!domainStats[domain]) {
        ...
        const newSuccessRate =
          (successCount / domainStats[domain].attempts) * 100;
        domainStats[domain].successRate = Math.min(
          Math.round(newSuccessRate * 10) / 10,
          100
        );
  ...
  const getRecommendedService = (
    ...
    if (
      ...
      analytics.domainStats[domain].attempts >= 3
    ) {
...
export function useBypassCache() {
  ...
  const getCachedService = (
    url: string
  ): { service: string; successful: boolean } | null => {
