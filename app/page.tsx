...
export default function Home() {
  ...
  const { getCachedService, cacheUrl } = useBypassCache();
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    try {
      ...
      if (cachedResult && cachedResult.successful) {
        ...
        const serviceName =
          cachedResult.service === "scribe"
            ? "Scribe.rip"
            : cachedResult.service === "12ft"
            ? "12ft.io"
            : cachedResult.service === "archive.is"
            ? "Archive.is"
        ...
        setLastRedirect(
          `${serviceResult.serviceName} (cached): ${serviceResult.serviceUrl}`
        );
      } else {
        // Check if analytics recommends a different service than the default
        const { serviceUrl, serviceName, serviceId } =
          determineBypassService(formattedUrl);
        ...
        if (recommendedService !== serviceId) {
          ...
          serviceResult = {
            ...
            serviceName:
              recommendedService === "scribe"
                ? "Scribe.rip"
                : recommendedService === "12ft"
                ? "12ft.io"
                : recommendedService === "archive.is"
                ? "Archive.is"
          ...
          setLastRedirect(
            `${serviceResult.serviceName} (recommended): ${serviceResult.serviceUrl}`
          );
        } else {
          // Use default service
    ...
    <div className="min-h-screen flex flex-col bg-white font-sans">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-normal text-gray-900">
              Bypass<span className="font-medium">Reader</span>
            </h1>
            ...
            <div className="mx-auto bg-white p-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label
                    htmlFor="url-input"
                    className="block text-left text-gray-600 text-sm font-medium mb-2"
                  >
                    Article URL
                  </label>
                ...
                <div className="mt-6 border-t border-gray-50 pt-4">
                  ...
                  {feedback === "success" && (
                    <p className="mt-2 text-sm text-gray-600">
                      Great! We will remember this for future articles.
                    </p>
                  )}
                  ...
                  {feedback === "failure" && (
                    <div className="mt-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          onClick={() =>
                            window.open(`https://archive.is/${url}`, "_blank")
                          }
                          className="px-3 py-1 bg-gray-100 text-gray-700 text-xs rounded hover:bg-gray-200 transition-colors"
                        >
                          Try Archive.is
                        ...
                        <button
                          onClick={() => {
                            try {
                              const urlObj = new URL(url);
                              window.open(
                                `https://scribe.rip${urlObj.pathname}`,
                                "_blank"
                              );
                            } catch {
                              window.open(
                                `https://scribe.rip/${url}`,
                                "_blank"
                              );
                            }
                          }}
                  ...
                  <tr>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {analytics.domainStats["medium.com"] ? (
                        <span>
                          {Math.min(
                            analytics.domainStats["medium.com"].successRate,
                            100
                          )}
                          %
                        </span>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {analytics.domainStats["nytimes.com"] ? (
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">
                          {Math.min(
                            analytics.domainStats["nytimes.com"].successRate,
                            100
                          )}
                          %
                        </span>
                  ...
                  <tr>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {analytics.domainStats["economist.com"] ? (
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">
                          {Math.min(
                            analytics.domainStats["economist.com"].successRate,
                            100
                          )}
                          %
                        </span>
                  ...
                  <tr>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {analytics.domainStats["wsj.com"] ? (
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">
                          {Math.min(
                            analytics.domainStats["wsj.com"].successRate,
                            100
                          )}
                          %
                        </span>
      ...
      <footer className="bg-white text-gray-500 py-8 border-t border-gray-200">
        <div className="container mx-auto px-4 text-center">
          <p className="text-sm">
            &copy; {new Date().getFullYear()} BypassReader. For educational
            purposes only.
          </p>
          <p className="text-sm mt-2">
            We do not host any bypassed content - we just point to the best
